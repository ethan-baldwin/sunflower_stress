rule fastp:
    ...
    resources:
        ...
        cpus_per_task=4
    shell:
        "fastp -w {resources.cpus_per_task} --dont_overwrite -gx --poly_x_min_len 8 --in1 {input.r1} --in2 {input.r2} --out1 {output.trimmed1} --out2 {output.trimmed2} --unpaired1 {output.unpaired1} --unpaired2 {output.unpaired2} --failed_out {output.failed} -j {output.json} -h {output.html}"